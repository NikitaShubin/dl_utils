# Модуль [`labels.py`](../labels.py "Перейти к модулю")

Предоставляет функциональность для работы с метками и суперметками,
хранящимися в Excel-файлах. Основное назначение - конвертация между
различными системами обозначений меток и организация иерархических связей
между ними.
...
Файл меток `labels.xlsx` представлен примером
[labels_template.xlsx](./labels_template.xlsx.md "Перейти к документации")

**Структура:**

- **Столбец A**: "Класс объекта" - иерархическая структура меток с нумерацией
...
Файл меток `superlabels.xlsx` представлен примером
[superlabels_template.xlsx](./superlabels_template.xlsx.md "Перейти к документации")
