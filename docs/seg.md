...
Предназначен для работы с сегментами и сегментацией изображений. Содержит инструменты для фильтрации, морфологической обработки, декомпозиции масок и работы с датасетами семантической сегментации.
...
| `tensor2masks()`                 | преобразование тензора обратно в список масок                                            |
| `DropByArea()`                   | создание фильтра по площади масок                                                        |
| `isolate_segments`               | разделение несвязных сегментов на отдельные маски                                        |
| `drop_shards`                    | оставление только самого большого сегмента из несвязных                                  |
| `fit_segments_in_df()`           | подгонка контуров сегментов в датафрейме CVAT                                            |
| `save_tasks2segmentation_dataset`| создание датасета сегментации из задач CVAT                                              |
...
### Класс `FitMasks`
Подгоняет контуры сегментов так, чтобы каждый пиксель изображения принадлежал только одной маске.
...
```python
save_tasks2segmentation_dataset(tasks, '/path/to/dataset', 
                               labels_convertor, make_preview=True)
```
