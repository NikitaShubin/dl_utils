# Модуль [`video_utils.py`](../video_utils.py "Перейти к модулю")

Предоставляет комплексные утилиты для работы с видео и изображениями,
включая:
...
- **обработка видеопотоков**: применение фильтров, наложение текста;
...
| `count_frames` | подсчёт реально декодируемых кадров |

Атрибут `total_frames` (и `__len__`) берётся из заголовка контейнера и может
завышать реальное число кадров на битых записях; для точного подсчёта
используется `count_frames()`. При сбое декодера `__call__` пропускает битые
пакеты и, если кадр так и не прочитан, возвращает `None` (или повторяет
последний кадр при `on_end='repeat_last_frame'`).
...
| `seq_flows`              | расчёт последовательных потоков         |
| `apply_flows2img`        | восстановление видео по потокам         |
