...
Jupyter AI или OpenCode для работы с моделями Ollama. Приоритет отдаётся использованию
внутренних ресурсов (Ollama в локальной сети) в том числе ради избегания утечек данных
на облачные ресурсы.
...
- Конфигурационный файл создаётся всегда (с настройками защиты от утечки данных).
  Если `OLLAMA_HOST` не задан или на серверах не найдено ни одной модели,
  файл создаётся без провайдеров — OpenCode запускается, но без локальных моделей.
