...
def read_superclasses2df(file):
    ...
    for ind in range(len(df)):
        ...
        # Если cur_superclass_name NaN, то он не равен сам себе, а, значит,
        # строка не дозаполнена:
        else:
...
def classes2tree(df):
    ...
    номера списков с вложенностью. Работает только на данных, полученных
    через read_full_label_names_df('classes_info/classes.xlsx').
    '''
...
class LabelsConvertor:
    ...
    '''
    def __init__(self                                                                                                             ,
                 classes_info_file: 'xlsx-файл, содержащий таблицу классов с привязкой к меткам в CVAT и GG' =      'classes.xlsx',
            superclasses_info_file: 'xlsx-файл, содержащий таблицу суперклассов с привязкой к классам'       = 'superclasses.xlsx'):
        ...
        self.cvat_meanings_list = [self.cvat_label2class_meaning[label.lower()]
                                   for label in self.classes[cvat_label_column]
                                   if pd.notna(label)]
        self.  gg_meanings_list = [self.   gg_uuid2class_meaning[label.lower()]
                                   for label in self.classes[uuid_label_column]
                                   if pd.notna(label)]
...
def apply_labels_convertor2tasks(tasks, labels_convertor):
    ...
    for task in tasks:
        ...
        for df, file, true_frames in task:
            df = df.copy()
            df['label'] = df['label'].apply(labels_convertor)
