...
class TestYOLOLabels:
    ...
    def test_apply_label_func(self):
    ...
    def test_save_with_excluded_label(self, tmp_path):
    ...
    def test_draw_labels(self):
        ...
        with patch('yolo.CVATPoints') as mock_cvat:
            ...
            mock_points2draw.draw.side_effect = lambda img, *args, **kwargs: img
    ...
    def test_draw_labels_with_args(self):
        ...
        def track_draw_calls(*args, **kwargs):
...
class TestStatisticFunctions:
    ...
    def test_class_statistic2superclass_statistic(self):
    ...
    def test_fill_skipped_rows_in_statistic(self):
...
class TestSources2Statistic:
    ...
    def test_cvat_source_split(self):
...
class TestYAMLGeneration:
    ...
    def test_gen_yaml(self, tmp_path):
        ...
        assert content['train'] == os.path.relpath(str(im_trn_dir), str(yolo_ds_dir))
        assert content['val'] == os.path.relpath(str(im_val_dir), str(yolo_ds_dir))
        assert content['test'] == os.path.relpath(str(im_tst_dir), str(yolo_ds_dir))
        assert content['names'] == {0: "animal", 1: "vehicle"}
        assert -1 not in content['names']  # Неиспользуемый класс исключен
    
    def test_gen_yaml_with_dir(self, tmp_path):
...
class TestTask2YOLO:
    ...
    def test_task2yolo_basic(self, mock_draw_text, mock_copyfile, mock_imwrite, 
                             mock_yolo_labels, mock_buffer, tmp_path):
        ...
        # Мокаем labels_convertor
        mock_convertor = Mock()
        mock_convertor.side_effect = lambda x: 0
    ...
    @patch('cv2.imwrite')
    @patch('cv2.resize')
    def test_task2yolo_with_scale(self, mock_resize, mock_imwrite, mock_yolo_labels, mock_buffer, tmp_path):
        ...
        with patch('yolo.CVATPoints') as mock_cvat:
            ...
            mock_cvat_instance.__mul__ = lambda self, k: self
    ...
    @patch('yolo.YOLOLabels')
    @patch('cv2.imwrite')
    def test_task2yolo_with_tiling(self, mock_imwrite, mock_yolo_labels, mock_split, mock_buffer, tmp_path):
...
class TestTasks2YOLO:
    ...
    def test_tasks2yolo_basic(self, mock_bal, mock_drop, mock_mpmap, mock_mkdirs, mock_init, 
                              mock_fill, mock_sort, mock_flat, tmp_path):
    ...
    @patch('yolo.drop_unused_track_ids_in_graphs')
    @patch('yolo.make_copy_bal')
    def test_tasks2yolo_with_copybal(self, mock_bal, mock_drop, mock_mpmap, mock_mkdirs, mock_init,
                                     mock_fill, mock_sort, mock_flat, tmp_path):
        ...
        tasks = [[(df, str(tmp_path / "test.jpg"), {0: 0})]]
